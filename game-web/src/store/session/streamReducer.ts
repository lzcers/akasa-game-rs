...
function areChoicesEqual(
  left: RoundState["choices"],
  right: RoundState["choices"],
): boolean {
...
export function reduceStreamEvent({
  ...
  if (event.type === "flow_turn_update") {
    ...
    if (
      event.stage === "simulation" &&
      event.output_type === "json" &&
      content
    ) {
...
function reducePlayerInput(
  ...
  const selectedChoiceText =
    ...
    previousRoundState?.choices.find((choice) => choice.action === actionText)
      ?.text ??
    action.title ??
    actionText;
...
function reduceNarrationText(
  ...
  const nextRoundState = createRoundState(round, {
    ...(previousRoundState ?? {}),
    round,
    title: previousRoundState?.title || "",
    narrationText,
    narrationStatus,
    isAwaitingNarration: false,
  });
  ...
  const internalStatePatch =
    ...
    previousRoundState?.isAwaitingNarration ===
      nextRoundState.isAwaitingNarration
      ? null
      : {
  ...
  const uiStatePatch =
    stateView && stateView.latestHistory !== narrationText
      ? {
          stateView: {
            ...stateView,
            phase:
              narrationStatus === "running" ? stateView.phase : "application",
...
function reduceWorldSnapshotEvent(
  ...
  const nextTitle = summary?.sceneTitle ?? previousRoundState?.title ?? "";
  let internalStatePatch: Partial<GameInternalState> | null = null;
  let uiStatePatch: Partial<StreamEventUIState> | null = null;
